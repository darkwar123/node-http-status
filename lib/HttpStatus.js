...
var HttpStatus = function (_Error) {
    ...
    function HttpStatus(error) {
        ...
        switch (typeof error === "undefined" ? "undefined" : _typeof(error)) {
            case 'object':
                if (error === null) {
                    break;
                }
        ...
        _this.status = Number(status) || 500;
        _this.message = typeof message === 'string' ? message : ERRORS[_this.status];

        _this.type = String(_this[getStatusType]());
        return _this;
    }
    ...
    _createClass(HttpStatus, [{
        ...
        value: function value() {
            var template = (Math.floor(this.status / 100) || 0) + 'xx';
            return TYPES[template];
        }
    }], [{
        key: "getInstance",
        value: function getInstance(error) {
